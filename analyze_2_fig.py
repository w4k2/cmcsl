...
for dataset_id, dataset in tqdm(enumerate(datasets), disable=True):
    for topic_id, topic in tqdm(enumerate(topics[dataset_id]), disable=False, total=len(topics[dataset_id])):
        ...
        for preproc_id, preproc in enumerate(preproc_names):
            ...
            for algorithm_id in range(len(alg_names)):
                ...
                if algorithm_id == len(alg_names)-1:
                    ax[2, preproc_id].plot(n_times, alg_scores_m1, c="red", ls="-.", lw=lw, label = "visual %s" % (alg_names[algorithm_id]))
                    ax[2, preproc_id].plot(n_times, alg_scores_m2, c="blue", ls="-.", lw=lw, label = "text %s" % (alg_names[algorithm_id]))
                else:
                    ax[2, preproc_id].plot(n_times, alg_scores_m1, c="red", lw=1, ls=ls[algorithm_id], label = "Visual %s" % ( alg_names[algorithm_id]))
                    ax[2, preproc_id].plot(n_times, alg_scores_m2, c="blue", lw=1, ls=ls[algorithm_id], label = "Text %s" % ( alg_names[algorithm_id]))
                    
                ax[2, preproc_id].spines[['right', 'top']].set_visible(False)
                ax[2, preproc_id].grid((.7, .7, .7), ls=":")
                ax[2, preproc_id].set_ylabel("Balanced accuracy", fontsize = 14)
                ax[2, preproc_id].set_xlabel("#samples for each class", fontsize = 14)
        ...
        lgnd = fig.legend(handles, labels, ncol=8, frameon=False, bbox_to_anchor=(0.5, 0.0), fontsize=17, loc='lower center')
        
        
        plt.savefig("figures/ex2/whole_%s_more_final.png" % topic)
        plt.savefig("figures/ex2/whole_%s_more_final.eps" % topic)
        # exit()