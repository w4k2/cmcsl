...
scores_m1 = scores_m1[[0, 10, 9, 19]]
scores_m2 = scores_m2[[0, 10, 9, 19]]
scores_lf_gnb = scores_lf_gnb[[0, 10, 9, 19]]
scores_ef_gnb = scores_ef_gnb[[0, 10, 9, 19]]
...
for topic_id, topic in tqdm(enumerate(topics[0]), total = 20):
    ...
    # PP-RAI 2024
    if topic_id in [0, 2, 4]:
        ax[topic_id].set_ylim(0.5, 1.0)
    else:
        ax[topic_id].set_ylim(0.1, 1.0)
    # ax.set_ylim(0.1, 1.0)
    ax[topic_id].set_xticks([i for i in n_times[::2]])
    ...
    ax[topic_id].plot(n_times, topic_scores_lf, c="black", ls="-", lw=1, label = "LF")
    ax[topic_id].plot(n_times, topic_scores_ef, c="black", ls="--", lw=1, label = "EF")
...
plt.savefig("figures/ex3/all_datesets_short.png", dpi=200)
plt.savefig("figures/ex3/all_datesets_short.eps", dpi=200)
plt.close()
