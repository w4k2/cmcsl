...
for dataset_id, dataset in tqdm(enumerate(datasets), disable=True):
    for topic_id, topic in tqdm(enumerate(topics[dataset_id]), disable=False, total=len(topics[dataset_id])):
        ...
        scores_combined = np.zeros((len(algorithms), n_times.shape[0]))
        
        for preproc in preproc_names:
            ...
            for times_id, times in enumerate(n_times):
                ...
                for algorithm_id in range(len(algorithms)):
                    ...
                    # pred_combined = clf.predict_combined([X_m1_test, X_m2_test])
                    # scores_combined[algorithm_id, times_id] = balanced_accuracy_score(y_test, pred_combined)
                    # ax.plot(n_times[:times_id+1], scores_combined[algorithm_id,:times_id+1], c="black", ls=ls[algorithm_id], label = "%s %s %.3f" % (modalities[dataset_id][0], alg_names[algorithm_id], np.mean(scores_combined[algorithm_id,:times_id+1])))
                ...
                plt.savefig("figures/ex2/run_%s_%s.png" % (topic, preproc), dpi=200)
                plt.close()
        # exit()